...
class URL {
	...
	get protocolString(){ return this.domain.length&&this.protocol.length?this.protocol+':':''; }
	get domainString(){ return this.domain.length?'//'+this.domain.join('.'):''; }
	get portString(){
	...
	get stringify() { return this.toString; }
}
...
const parseExternalMedia = async function parseExternalMedia(url) {
	...
	switch (found) { // meta: title size duration dims
		...
		case 'youtube':
			...
			break;
		case 'dailymotion':
			id = m.domainString.slice(2)=='dai.ly'?m.uri[0]:m.uri[1].split('_')[0];
...
_.processMarkup = function(markdown, ticker=false){
	...
	let keys = Config.cfg.markdown.custom, ignore = Config.cfg.markdown.ignore||null, 
		lists = Config.cfg.markdown.lists||[], hlink = Config.cfg.markdown.hyperlink,
		markup = markdown.replace(new RegExp("\r",'g'),'').replace(new RegExp("\0",'g'),''), 
		cursor = 0, links = [], 
		depth = [], track = [], 
		listdepth = [], listtrack = [];
	...
	parseloop: while (1){
		...
		let i = -1, skip = true;
		let linestart = cursor == 0 || markup.substr(cursor-1,1)=="\n";
		...
		if (!ticker && lists){
			...
			let	openlist = (list,indent)=>{ // track list bookmark location
				let current = {list,indent,start:cursor,itemstart:null,count:0};
				listdepth.push(current);
				// TODO: decide - consume newlines before the bookmark?
				openlistitem(current); // implicitly open list item
			};
			
			let openlistitem = (current,indent)=>{ // track list item bookmark location
				current.itemstart = cursor;
				current.count = 0;
				// remove indent + key + ' '
				markup = markup.splice(cursor,indent.length+current.list.key.length+1);
			};
			
			let closelistitem = (current)=>{ // inject list item bookmarks
				let a = "\rli\0\0\r", b = "\r\0\0li\r";
				while(current.count--) // prevent other markup from going across list items
					depth.pop().start.pop();
				markup = markup.splice(cursor,1,a); // remove the newline after a li close
				markup = markup.splice(current.itemstart,0,b);
				cursor += a.length-1+b.length + 1;
				current.itemstart = null;
			};
				
			let closelist = (current,EOF=false)=>{ // inject list bookmarks
				// inject @ cursor -1 (before the newline split)
				closelistitem(current); // implicitly close list item
				let i = lists.indexOf(current.list);
				let a = "\r"+i+"\0\0\0\r", b = "\r\0\0\0"+i+"\r";
				if (EOF)
					markup = markup.splice(cursor,0,a);
				else 
					markup = markup.splice(cursor-1,0,a);
				markup = markup.splice(current.start,0,b);
				cursor += a.length+b.length;
				listdepth.pop();
				listtrack.push(current.list);
			};
			
		
		if (listdepth.length && cursor >= markup.length) { // end of text within a list
			closelist(listdepth[listdepth.length-1],true);
			
		} else if (listdepth.length && markup.substr(cursor,1) == "\n") { // end of line within a list
			closelistitem(listdepth[listdepth.length-1]);
			
		} else if (cursor == 0 || markup.substr(cursor-1,1) == "\n") { // start of line, check for list keys
			let indent = markup.slice(cursor,markup.slice(cursor).indexOf("\n")).match(/^\t*|^ */)[0];
			let current = listdepth[listdepth.length-1];
			let i = -1;
			listloop: while (++i < lists.length) { // loop through keys
				let list = lists[i];
				if (markup.substr(cursor,indent.length+lists[i].key.length+1) == indent+lists[i].key+' ') { // found indent + list key + separator space
					if (current == null) { // not within a list, start list
						openlist(list);
						skip = false;
						break listloop;
					} else if (indent == current.indent) { // same indent
						if (list.key == current.list.key) { // same key
					...
					} else if (indent.length > current.indent.length && indent.startsWith(current.indent)) { 
						// == larger indent, validate indent type
						
						// TODO
						break listloop;
					} else if (indent.length < current.indent.length && current.indent.startsWith(indent)) { 
						// == smaller indent, validate indent type
						
						// TODO
						break listloop;
					}
					
				} else if (listdepth.length) {
					
					if (cursor>=markup.length || markup.substr(cursor,1) == "\n") { 
					 	// == no content found on the current line or end of markup
					 
					 	let i = listdepth.length;
					 	while (--i >= 0) 
					 		closelist(listdepth[i],cursor>=markup.length);
				 		break listloop;
				 	} else if (indent.length >= current.indent.length && indent.startsWith(current.indent)) { 
				 		// == same or larger indent, validate indent type
						// remove the indent value, leave the line break for <br> element
						markup = markup.splice(cursor,current.indent.length);
						// skip the rest of the found indent
						cursor += indent.length-current.indent.length;
						skip = false;
						break listloop;
					} else if (indent.length < current.indent.length && current.indent.startsWith(indent)) { 
						// == smaller indent, validate indent type
						// close lists in loop until equivalent or less indent length is found
					 	let i = listdepth.length;
					 	while (--i >= 0) {
					 		let cur = listdepth[i];
					 		// existing intent is smaller, leave as current
					 		if (indent.length >= cur.indent.length) break; 
					 		closelist(cur);
					 	}
					 	cursor += indent.length; // move cursor past the indent
						break listloop;
					}
				}
			}
		...
		// let keyfound = false, indent = linestart?markup.slice(cursor-1,markup.slice(cursor).indexOf("\n")).match(/^\t*|^ */)[0]:'';
		// let i = -1;
		// while (cursor < markup.length && ++i < lists.length) { // speed check for a list key match.
		// 	keyfound = linestart && lists[i].key && markup.substr(cursor,indent.length+lists[i].key.length+1) == indent+lists[i].key+' ';
		// 	console.log('precheck',keyfound,linestart,lists[i].key.length,markup.substr(cursor,indent.length+lists[i].key.length+1)==indent+lists[i].key+' ');
		// 	if (keyfound) break;
		// }
		// if (linestart) console.log('check',keyfound,linestart,listdepth.length);
		if (keyfound || (linestart || cursor >= markup.length) && listdepth.length){ // proceed if key is present or inside list
			
			
			let current = listdepth.length?listdepth[listdepth.length-1]:null;
			let i = -1;
			while (++i < lists.length) {
				let list = lists[i];
				if (keyfound) { 
					// == Key found (from speed check)
					if (current == null) { 
						// == not within a list, start list
						openlist(list);
						skip = false;
						break;
					} else if (indent == current.indent) { 
						// == same indent
						if (list.key == current.list.key) { 
							// == same key
					...
					// == Key not found, but within list
					 if (markup.substr(cursor-1,2) == "\n\n"||cursor>=markup.length) { 
					 	// == no content found on the current line or end of markup
					 
					 	let i = listdepth.length;
					 	while (--i >= 0) 
					 		closelist(listdepth[i]);
				 		break;
				 	} else if (indent == current.indent || indent.length > current.indent.length && indent.startsWith(current.indent)) { 
				 		// == same or larger indent, validate indent type
						// remove the indent value, leave the line break for <br> element
						markup = markup.splice(cursor,current.indent.length);
						// skip the rest of the found indent
		...
		if (e && e.exclusivetext && e.after && markup.substr(cursor,e.after.length) == e.after){
			...
			let startindex = e.start[e.start.length-1] + b.length;
			...
			let endindex = cursor - a.length;
			if (e.filter && _.exists('/lib/modules/'+e.filter)) {
				let prefilter = markup.slice(startindex,endindex);
				let filtered = require(path('/lib/modules/'+e.filter))(prefilter);
				markup = markup.splice(startindex,endindex-startindex,filtered);
				cursor = cursor - (endindex-startindex) +filtered.length;
			}
			e.start.pop();
			track.push(e);
			depth.pop();
		...
		while (++i < keys.length){
			...
			if (t.after && depth.indexOf(t) != -1 && markup.substr(cursor,t.after.length) == t.after) {
				...
				let startindex = t.start[t.start.length-1] + b.length;
				...
				cursor = cursor - nlafter + a.length;
				let endindex = cursor - a.length;
				if (t.filter && _.exists('/lib/filters/'+t.filter)) {
					let prefilter = markup.slice(startindex,endindex);
					let filtered = require(path('/lib/filters/'+t.filter))(prefilter);
					markup = markup.splice(startindex,endindex-startindex,filtered);
					cursor = cursor - (endindex-startindex) +filtered.length;
				}
				depth.splice(depth.lastIndexOf(t));
				t.start.pop();
				if (listdepth.length && listdepth[listdepth.length-1].count) 
					listdepth[listdepth.length-1].count--;
				track.push(t);
				skip = false;
				break;
		...
		if (skip) cursor++;
		if (cursor > markup.length) break parseloop;
	}
	// \r (aka &#13;) and \0 (aka &#0;) are the only characters that are guarenteed not to be present before and after processing
	...
	// Also prevents malicious html code from leaking through. Padded with \0 for type identification.
	
	// Sanitize the original input (with bookmarks)
	markup = encoder.htmlEncode(markup.replace(/^\n[\n\t ]+$/g,"\n"));
	
	// insert html for any lists found
	console.log(listdepth);
	console.log(markup.substr(listdepth[0].itemstart-5,10));
	console.log();
	console.log(markup);
	let i = -1;
	while(++i < listtrack.length) {
		let k = lists.indexOf(listtrack[i]);
		markup = markup
			.replace(new RegExp("&#13;&#0;&#0;&#0;"+k+"&#13;",'g'),'<ul class="'+listtrack[i].class+'">')
			.replace(new RegExp("&#13;"+k+"&#0;&#0;&#0;&#13;",'g'),'</ul>');
	}
	markup = markup
		.replace(new RegExp("&#13;&#0;&#0;li&#13;",'g'),'<li>')
	...
	// insert html for any custom markup items found
	i = -1;
...
const RegFlag = (flag,board)=>{
	let d = -1, current = board == '_'?Config.globalflags:Config.flags;
	while (++d < flag.length){
	...
	Config.yml.write(Config.lib.path(board=='_'?'/flags.global.yml':'/flags.yml'),Config.flags,()=>{});
};
User.prototype.auth = function(flag,def,ault){
	...
	let i = -1,pass=[];
	// loop to allow for checking against an array of different flags.
	while (++i < flag.length) {
		...
		let f = flag[i].split('-');
		if (f.length == 1) f = flag[i].split('.'); // backwards-compatibility, DEPRECIATED
