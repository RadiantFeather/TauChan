...
_.getSpoiler = function(board,size){
	if (!size || (board.spoilerimg && board.spoilerimg != "/_/spoiler.png")) return Config.cdn+board.spoilerimg;
	// autosize the values to be proportional to 200px
	let s = size.split('x');
	if (s[0] > s[1]){ s[1] = parseInt(200 * s[1] / s[0],10); s[0] = 200; } 
	else if (s[1] > s[0]){ s[0] = parseInt(200 * s[0] / s[1],10); s[1] = 200; } 
	else { 	s[0] = 200; s[1] = 200; }
	if (s[0] > 200) s[0] = 200;
	if (s[1] > 200) s[1] = 200;
	console.log('dyn spoiler fetch', s);
	if (_.exists(__dirname+"/../assets/_/media/spoiler."+s[0]+"x"+s[1]+".png"))
		return Config.cdn+"/_/media/spoiler."+s[0]+"x"+s[1]+".png";
...
const parseExternalMedia = async function parseExternalMedia(url) {
	...
	switch (found) { // meta: title size duration dims
		...
		case 'youtube':
			...
			if (!id) throw new Error('No dicernable video ID was given.');
			r.mediatype = 'you';
			...
			try{
				let res = await request.get('https://youtube.com/get_video_info?video_id='+id,{timeout:5});
		...
		case 'dailymotion':
			...
			r.hash = Crypto.createHash('md5').update(found+'&'+id).digest('hex');
			try {
				let res = await request('https://api.dailymotion.com/video/'+id+'?fields=title,thumbnail_120_url,duration',{timeout:5});
				let data = qs.parse(res);
...
// Secure string for securing forms and links
...
const RegFlag = (flag)=>{
	let d = -1, level = Config.flags;
	while (++d < flag.length){
		if (!level[flag[d]]) level[flag[d]] = {};
		level = level[flag[d]];
	}
	level[flag[d]] = 'The flag '+flag.join('.')+' has not yet been described.';
	if (Config.cfg.devmode) console.log('Please define flag: '+flag.join('.'));
	Config.yml.write('./flags.yml',Config.flags,()=>{});
};
...
// User Cookie: ctx.cookies.set('user',req.session.id,{httpOnly:true,maxAge:1000*60*60*24*7});
_.User = User;
